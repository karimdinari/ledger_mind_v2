...
4. [Prerequisites & local setup](#4-prerequisites--local-setup) ← **Docker quick start**
...
Only **Docker Desktop** (or Docker Engine + Compose) is required — no local Python / Node / Mongo install.
...
# edit .env — set at least GEMINI_API_KEY and MISTRAL_API_KEY
...
Optional — seed the education RAG corpus (needs `MISTRAL_API_KEY`):

```bash
docker compose --profile seed run --rm seed-corpus
```
