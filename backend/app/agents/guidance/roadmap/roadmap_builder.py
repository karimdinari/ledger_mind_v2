...
def _question_manquante(analyse) -> str | None:
    if analyse.durabilite != AJ.DURABILITE_INDET:
        return None
    if analyse.categorie == "mixte" and not (
        analyse.ca_retenu.ca_prestations > 0 and analyse.ca_retenu.ca_vente > 0
    ):
        return (
            "Peux-tu préciser la répartition de ton chiffre d'affaires entre tes prestations "
            "de services et ta vente de produits ?"
        )
    return (
        "Ton chiffre d'affaires de l'an dernier dépassait-il déjà le plafond micro ? "
