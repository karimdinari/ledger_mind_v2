...
import { fetchRoadmapState, saveRoadmapState } from "@/lib/guidance-api";
...
function ResultatPage() {
  ...
  /** Coche optimiste puis persistance serveur, comme dans la conversation : la progression est
   *  la même donnée d'un écran à l'autre. En cas de refus serveur on revient en arrière et on le
   *  signale, plutôt que de laisser une coche qui s'évapore au rechargement. */
  const toggleStep = (id: string) => {
    if (!sessionId) return;
    const precedent = checked;
    const suivant = { ...checked, [id]: !checked[id] };
    setChecked(suivant);
    setSaveError(null);
    void saveRoadmapState(sessionId, suivant).catch(() => {
      setChecked(precedent);
      setSaveError("Progression non enregistrée. Reconnectez-vous, puis réessayez.");
    });
  };
    ...
    <div className="min-h-screen px-6 py-16 max-w-6xl mx-auto">
      ...
      {!detail && !error ? (
        ...
        <>
          ...
          {/* Progression : où en est l'utilisateur dans sa feuille de route, et de quoi cocher
              une étape sur place — c'est l'écran qu'on ouvre pour la parcourir. */}
          {roadmap && (roadmap.etapes?.length ?? 0) > 0 && (
            <div className="mt-6">
              ...
              <RoadmapStepper roadmap={roadmap} checked={checked} onToggle={toggleStep} />
              {saveError && (
                <p role="status" className="mt-3 font-mono text-sm text-destructive">
                  {saveError}
                </p>
              )}
            </div>
