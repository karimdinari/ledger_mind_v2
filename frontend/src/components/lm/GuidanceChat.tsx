...
export function GuidanceChat() {
   ...
   *
   *  Si le serveur refuse (session expirée le plus souvent), on revient à l'état précédent et on
   *  le dit : une coche qui s'affiche puis disparaît au rechargement est pire que pas de coche. */
  const persistChecks = (next: Record<string, boolean>, previous: Record<string, boolean>) => {
    if (!sessionId) return;
    void saveRoadmapState(sessionId, next).catch(() => {
      setChecked(previous);
      setError("Progression non enregistrée. Reconnectez-vous, puis réessayez.");
    });
  };
  ...
  const toggleStep = (id: string) => {
    ...
    setChecked(next);
    persistChecks(next, checked);
  };

  const resetChecks = () => {
    const previous = checked;
    setChecked({});
    persistChecks({}, previous);
  };
